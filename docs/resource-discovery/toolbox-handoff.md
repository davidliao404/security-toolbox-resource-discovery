...
资源发现网关团队拥有第一版接口定义权。外部工具箱团队必须适配 `toolbox-api-contract.md` 中的路径、签名、状态、分页和错误模型；联调用于验证工具箱实现是否符合当前网关协议，不用于默认重谈字段、路径或状态语义。详细规则见 `docs/resource-discovery/toolbox-integration-rules.md`。
